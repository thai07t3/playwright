...
export class PersonRepository {
    ...
    try {
      ...
      return data.map((personData, index) => {
        try {
          ...
          throw new Error(
            `Invalid person data at index ${index}: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      });
    } catch (error) {
  ...
  async savePeople(
    people: Person[],
    filename: string = "people.output.json",
  ): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to save people to ${filePath}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
}
